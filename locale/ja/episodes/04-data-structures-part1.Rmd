...
何が起こったのでしょう？
The `cats` (and `cats2`) data we are working with is something called a _data frame_. データフレームは、R で最も一般的で多用途な _データ構造_ の 1 つです。
...
さまざまなデータ構造とそれらの振る舞いについて詳しく見てみましょう。 For now, we will focus on our original data frame `cats` (and we can forget about `cats2` for the rest of this episode).
