...
important concept in programming called _data types_. We can ask what type of
data something is:
...
What happened?
The `cats` (and `cats2`) data we are working with is something called a _data frame_. Data frames
...
In this case, R cannot store everything in the data frame column `weight` as a _double_ anymore once we add the row for the additional cat (because its weight is `2.3 or 2.4`), therefore the entire
...
condition that _everything in the vector must be the same basic data type_. If
...
we see that `cats$weight` is a vector, too - _the columns of data we load into R
data.frames are all vectors_, and that's the root of why R forces everything in
...
`complex` -> `character`, where -> can be read as _are transformed into_. For
...
want in it. Remember _everything in the vector must be of the same basic data type_,
